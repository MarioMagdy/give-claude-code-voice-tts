...
Speech is easier to judge with your ears than from a README. There's a playable demo page — real
`edge-tts` audio for nine of the shipped voices, plus a worked example where the code block and the
results table stay silent while only the one-line summary is read aloud:

**[▶ mariomagdy.github.io/give-claude-code-voice-tts](https://mariomagdy.github.io/give-claude-code-voice-tts/)**

Source and audio live in [`docs/`](docs/). Every clip was generated by the same `edge-tts` engine
the plugin ships with, using voice ids from the curated table in `hooks/voices.py` — it's the real
output, not a re-recording.
